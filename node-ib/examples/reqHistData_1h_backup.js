...
var test_order= require('./utils/test_order.js');
...
collection=db.collection('reqHistoryData');
    ...
    if (data.change_stop == 'yes'){
      client.hget("keep_stops",data.name,function(error,reply){
         console.log('coin name is: ' + data.name + ' order kept is: '  + reply);
         ib.cancelOrder(Number(reply));
      });

      test_order.test_order_func(data.signal,data.close,function(error,result){
      console.log("result is bb:" + result);
      setTimeout(function(){ ib.placeOrder(++orderid,ib.contract.forex(data.name),ib.order.stop(data.signal,50000,Number(result)));
         client.hset('keep_stops',data.name,orderid); } ,1000);
      });
        ...
        test_order.test_order_func(data.signal,data.stop,function(error,result){
        console.log("result is bb:" + result);
        //   fixed_data_close=result;
        setTimeout(function(){ ib.placeOrder(++orderid,ib.contract.forex(data.name),ib.order.stop(opposite_signal(data.signal),50000,Number(result)));
              client.hset('keep_stops',data.name,orderid); } ,1000);
        });
    }

//  console.log('fixed data close is: ' + fixed_data_close);
//  ib.placeOrder(++orderid,ib.contract.forex(data.name),ib.order.stop(test_order.test_order_func(opposite_signal(data.signal),50000,fixed_data_close)));
...
ib.reqHistoricalData(1,  ib.contract.forex('GBP'),dateformat(now,"yyyymmdd H:MM"),durationStr='1400 S',barSizeSetting='1 hour',whatToShow='MIDPOINT',useRTH=1,formatDate=1);
ib.reqHistoricalData(2,  ib.contract.forex('EUR'),dateformat(now,"yyyymmdd H:MM"),durationStr='1400 S',barSizeSetting='1 hour',whatToShow='MIDPOINT',useRTH=1,formatDate=1);
ib.reqHistoricalData(3,  ib.contract.forex('CAD'),dateformat(now,"yyyymmdd H:MM"),durationStr='1400 S',barSizeSetting='1 hour',whatToShow='MIDPOINT',useRTH=1,formatDate=1);
ib.reqHistoricalData(4,  ib.contract.forex('JPY'),dateformat(now,"yyyymmdd H:MM"),durationStr='1400 S',barSizeSetting='1 hour',whatToShow='MIDPOINT',useRTH=1,formatDate=1);
ib.reqHistoricalData(5,  ib.contract.forex('AUD'),dateformat(now,"yyyymmdd H:MM"),durationStr='1400 S',barSizeSetting='1 hour',whatToShow='MIDPOINT',useRTH=1,formatDate=1);
ib.reqHistoricalData(6,  ib.contract.forex('CHF'),dateformat(now,"yyyymmdd H:MM"),durationStr='1400 S',barSizeSetting='1 hour',whatToShow='MIDPOINT',useRTH=1,formatDate=1);
ib.reqHistoricalData(7,  ib.contract.forex('NZD'),dateformat(now,"yyyymmdd H:MM"),durationStr='1400 S',barSizeSetting='1 hour',whatToShow='MIDPOINT',useRTH=1,formatDate=1);
ib.reqHistoricalData(8,  ib.contract.forex('KRW'),dateformat(now,"yyyymmdd H:MM"),durationStr='1400 S',barSizeSetting='1 hour',whatToShow='MIDPOINT',useRTH=1,formatDate=1);
