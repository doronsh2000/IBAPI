 ...
 // setInterval(function(){
    ...
    event.on('forex_buy_and_sell:app',function(data){
      ...
      if(check_if_buy_or_sell_in_place==data.signal && coin==data.name ){
		...
		ib.placeOrder(orderid++,ib.contract.forex(data.name),ib.order.market(data.signal,100000));
             ...
             ib.placeOrder(orderid++,ib.contract.forex(data.name),ib.order.market(data.signal,100000));
             fs.appendFileSync('/IB_TI/node-talib-0.4.0/examples/signals.txt',data.name + '-' + data.signal + ' bought 100000' + ' '+ new Date()  + '\n');
             }
         } else {
             //  ib.cancelOrder(orderid);
              // ib.cancelOrder(orderid-1);
              // trail_orderid=orderid+1;
              ib.placeOrder(orderid++,ib.contract.forex(data.name),ib.order.market(data.signal,100000));
              client.hmset(data.name+".coin","name",data.name,"amount",50000,"close",data.close,"signal",data.signal,redis.print);
