...
var coin_to_reqid_map={"1": "GBP", "2":"EUR", "3":"CAD","4":"JPY","5":"AUD","6":"CHF"};
      ...
      if(check_if_buy_or_sell_in_place==data.signal && coin==data.name ){
         ...
         if(fs.existsSync('/IB_TI/node-talib-0.4.0/examples/' + data.name + '.first')){
           ...
           //  trail_orderid=orderid+1;
             //ib.placeOrder(orderid++,ib.contract.forex(data.name),ib.order.market(data.signal,200000));
             fs.appendFileSync('/IB_TI/node-talib-0.4.0/examples/signals.txt',data.name + '-' + data.signal + ' bought 200000' + ' '+ new Date()  + '\n');
...
ib.reqHistoricalData(1,  ib.contract.forex('GBP'),dateformat(now,"yyyymmdd H:MM"),durationStr='60 S',barSizeSetting='5 secs',whatToShow='MIDPOINT',useRTH=1,formatDate=1);
