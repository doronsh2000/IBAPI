...
var coin_to_reqid_map={"1": "GBP", "2":"EUR", "3":"CAD"};
...
// bellow is 1h reqHistory to see the overall trend. mimics do-loop. trigger first time manually. then repeat every 1h.

var now=new Date();console.log(dateformat(now,"yyyymmdd H:MM:ss"));
ib.reqHistoricalData(3,  ib.contract.forex('CAD'),dateformat(now,"yyyymmdd H:MM"),durationStr='3500 S',barSizeSetting='1 hour',whatToShow='MIDPOINT',useRTH=1,formatDate=1);
/*
ib.reqHistoricalData(1,  ib.contract.forex('GBP'),dateformat(now,"yyyymmdd H:MM"),durationStr='60 S',barSizeSetting='1 min',whatToShow='MIDPOINT',useRTH=1,formatDate=1);
ib.reqHistoricalData(2,  ib.contract.forex('EUR'),dateformat(now,"yyyymmdd H:MM"),durationStr='60 S',barSizeSetting='1 min',whatToShow='MIDPOINT',useRTH=1,formatDate=1);
...
// bellow is 1min reqHistory. mimics do-loop. trigger first time manually. then repeat every 1min.
...
event.on('ready',function(){
   event.on('forex_buy_and_sell:app',function(data){
      ...
      if(check_if_buy_or_sell_in_place==data.signal && coin==data.name){
         ...
         if (data.signal=='BUY'){
            ib.placeOrder(orderId+1,ib.contract.forex(data.name),ib.order.trailingStop('SELL',1000,0.00100));
         }else {
            ib.placeOrder(orderId+1,ib.contract.forex(data.name),ib.order.trailingStop('BUY',1000,0.00100));
         }
