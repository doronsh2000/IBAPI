 ...
 // setInterval(function(){
    ...
    event.on('forex_buy_and_sell:app',function(data){
      ...
      if(check_if_buy_or_sell_in_place==data.signal && coin==data.name ){
       ...
       //  ib.placeOrder(orderId,ib.contract.forex(data.name),ib.order.market(data.signal,100));
         if (data.signal=='BUY'){
            console.log('placing a SELL trail stop on' + ib.contract.forex(data.name));
        //    ib.placeOrder(orderId+1,ib.contract.forex(data.name),ib.order.trailingStop('SELL',100,0.00100));
         }else {
            console.log('placing a BUY trail stop on' + ib.contract.forex(data.name));
        //    ib.placeOrder(orderId+1,ib.contract.forex(data.name),ib.order.trailingStop('BUY',100,0.00100));
         }
