...
// var event=new RedisEvent('localhost',['forex_buy_and_sell']);
  event.on('ready',function(){
    event.on('forex_buy_and_sell:app',function(data){
       console.log(data);
       array_data_entry[array_data_entry.length]=data;
});
});

   if(array_data_entry.length >0){
     array_data_entry.forEach(function(data_entry){
       console.log('data_entry is: ' + data_entry + ' signal is: ' + data_entry.signal + ' coin is: ' + data_entry.name );
       if(data_entry.reqid==reqId && close - data_entry.start_position_close > -0.00100 && data_entry.signal=='BUY' ){
         console.log('exiting!! i want to cut my losses !! exit from trade-!!'  + data_entry.name);
             ib.placeOrder(orderid++,ib.contract.forex(data_entry.name),ib.order.market('SELL',50000));
             fs.appendFileSync('./signals.txt','exiting!! i want to cut my losses !! exit from trade ' + data_entry.name + '-' + data_entry.signal + ' bought 50000' + ' '+ new Date() + '\n');
                fs.exists('/IB_TI/node-talib-0.4.0/examples/' + data_entry.name + '.first' ,function(exists){
                if(exists){
                  fs.unlinkSync('/IB_TI/node-talib-0.4.0/examples/' + data_entry.name + '.first');
                }
                });

          data_entry.reqid='';
       }
       if(data_entry.reqid==reqId &&  data_entry.start_position_close - close > -0.00100 && data_entry.signal=='SELL' ){
          console.log('exiting!! i want to cut my losses !! exit from trade-' + data_entry.name);
             ib.placeOrder(orderid++,ib.contract.forex(data_entry.name),ib.order.market('BUY',50000));
                fs.appendFileSync('./signals.txt','exiting!! i want to cut my losses !! exit from trade ' + data_entry.name + '-' + data_entry.signal + ' bought 50000' + ' '+ new Date() + '\n');
                fs.exists('/IB_TI/node-talib-0.4.0/examples/' + data_entry.name + '.first' ,function(exists){
                if(exists){
                  fs.unlinkSync('/IB_TI/node-talib-0.4.0/examples/' + data_entry.name + '.first');
